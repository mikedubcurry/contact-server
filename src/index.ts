  ...
  async (req, res) => {
    ...
    if (!name || !email || !message) {
      return res.status(400).json({
        status: "error",
        message: "must send name, email and message",
      });
    }
    ...
    if (message.length > 240) {
      return res
        .status(400)
        .json({ status: "error", message: "message exceeds allowed length" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ status: "error", message: "Do not send URLs" });
    }
    ...
    try {
      ...
      if (messageSent.rejected.length) {
        return res
          .status(500)
          .json({ status: "error", message: "message not sent" });
      }
      if (messageSent.response.includes(" OK ")) {
        ...
        throw new EmailFailedError("email failed to send");
      }
    } catch (e) {
      return res
        .status(500)
        .json({ status: "error", message: "error in POST /sendMail", e });
    }
  }
);
...
app.post("/blockUser", (req, res) => {
  ...
  if (!ip) {
    return res.status(400).json({
      status: "error",
      message: "must supply ip to be added to blockList",
    });
  }
  addToBlockList(ip);
